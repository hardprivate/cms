...
class Model_Video extends Model
{
	...
	public function get_data()
	{
		...
		$nameArr = array('content','donat', 'view', 'unlike', 'like' , 'download', 'posts', 'mTorrent');
		...
		if ($this->CheckIssetVideo($videoOut))
		{
			...
			// 5 реквизиты для доната:
			array_push($data, $this->Donate($videoOut) );
			
			//скрипт всплывающего окна на странице
			$this->Show_window();
	...
	private function Donate($videoOut)
	{
		if (is_array($videoOut) && $videoOut['video_level'] > 0)
		{
		$query = 'SELECT 
			`donat_id`, 
			`donat_type`, 
	...
	/////////////////////////////////////////////////////////////////////////////
	private function Show_window()
	{
		// jQuery
		array_push(ConfigHTML::$script_src_array, '/js/jquery-3.6.0.min.js');
		
		// Плавное появление элемента при загрузке (не работает)
		//array_push(ConfigHTML::$script_src_array, '/js/plavnoe_poyavlenie_elementa.js');
		//array_push(ConfigHTML::$css_src_array, '/css/plavnoe_poyavlenie_elementa.css');
		
		// При клике создаст окно с содержимым реквизитов для доната
		array_push(ConfigHTML::$script_src_array, '/js/new_window_popup.js');
		array_push(ConfigHTML::$css_src_array, '/css/new_window_popup.css');

		// При клике копирует текст в буфер
		array_push(ConfigHTML::$script_src_array, '/js/copytext.js');

		/*
		array_push(ConfigHTML::$css_src_array, '/js/show_pict/template.css');
		array_push(ConfigHTML::$script_src_array, '/js/show_pict/mootools.js');
		array_push(ConfigHTML::$css_src_array, '/js/show_pict/fetchscript.css');
		array_push(ConfigHTML::$script_src_array, '/js/show_pict/fetchscript.js');
		*/
	}
	...
	private function PlayerJS()
	{
		...
		// array_push(ConfigHTML::$css_src_array, '/css/video-js.css');
		// array_push(ConfigHTML::$script_src_array, '/js/video.js');
	}
	...
	private function VideoSetLinkDowload($massivSQL)
	{
		if (isset(Names::$n_reuestUriArray[3]) && ((Names::$n_reuestUriArray[3] == 'content')))  //|| (Names::$n_reuestUriArray[3] == 'download')) )
		{
			...
			header('Content-Disposition: attachment; filename="'.$massivSQL['video_id'].'.'.$massivSQL['video_mp4'].'"');
		...
		if (isset(Names::$n_reuestUriArray[3]) && (Names::$n_reuestUriArray[3] == 'mTorrent')) 
		{
			//header('Content-type:application/torrent');
			header('Content-type: application/octet-stream'); 
			header('Content-disposition: attachment; filename="HardPrivate.com.'.$massivSQL['video_id'].'.torrent"');
			//header('Content-disposition: inline');
			header("Content-Transfer-Encoding: binary");
			//header("Location: ".Names::$n_reuestUriArray[0]."/video/".$massivSQL['videodisck_name']."/torrent/".$massivSQL['video_name'].".torrent");	
			header("Content-Length: ".filesize($massivSQL['video_link_mTorrent'].$massivSQL['video_name'].".".$massivSQL['video_mp4'].".torrent"));
			$handle = fopen($massivSQL['video_link_mTorrent'].$massivSQL['video_name'].".".$massivSQL['video_mp4'].".torrent", "rb") or die("Couldn't get handle");
			if ($handle) {
				while (!feof($handle)) {
	...
	*/
	...
	private function OutputVideo()
	{
		...
		$query = "SELECT
		 ...
		 `".ConfigMySQL::db_prefix."video`.`video_mp4` AS video_mp4 ,
		 ...
		 `".ConfigMySQL::db_prefix."video`.`video_mTorrent` AS video_mTorrent ,
		 ...
		 `".ConfigMySQL::db_prefix."video`.`video_level` AS video_level ,
	...
	private function OutputRandom4()
	{
		...
		$query = "SELECT
		 ...
		 `".ConfigMySQL::db_prefix."video`.`video_mp4` , 
		  `".ConfigMySQL::db_prefix."video`.`videosubdir_id` AS videosubdir_id ,
	...
	--	 `".ConfigMySQL::db_prefix."video`.`video_kategory` , 
		 ...
		 `".ConfigMySQL::db_prefix."video`.`video_mTorrent` ,		 
	...
	private function MassivLink(&$massiv)
	{
		...
		// Преобразуем ссылку на видео на локальный сервер
				...
				$massiv['video_link_abs'] = '/home/gt/html/ha0/public_html/video/'.
					$massiv['videodisck_name'] . '/' . $massiv['video_type'] . '/' .
					$massiv['videodir_dir'] . '/';
				$massiv['video_link_mTorrent'] = '/home/gt/html/ha0/public_html/video/'.
					$massiv['videodisck_name'] . '/torrent/';
		...
		// Картинка постера на локальный сервер:
				...
				$massiv['video_link'] .= $massiv['video_name'].'.'.$massiv['video_mp4'];
				$massiv['video_link_abs'] .= $massiv['video_name'].'.'.$massiv['video_mp4'];
